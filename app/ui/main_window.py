...
主窗口界面结构（专业化停靠布局）：
    - 菜单栏 + 工具栏（含第二行导出按钮栏）
    - 左侧停靠栏（占整列）：工作区目录树 / 原图预览 / 图像信息
    - 中央：主视图窗（图层滚轮切换）+ 缩放控制 + 数据采集
...
from app.ui.layer_viewer import ImageViewerEx
from app.ui.step_indicator import StepIndicator, panel_title
from app.ui.camera_dialog import CameraDialog  # noqa: F401  (供外部统一导入)
from core.grading import SIEVE_SIZE
from app.config import FontSize
from app.styles import THEMES


class MainWindow(QMainWindow):
    """主窗口骨架：只负责界面构建与基础联动，业务逻辑在 app.controllers 中。"""
    ...
    def _build_left_dock(self):
        ...
        wsHead = QHBoxLayout()
        wsHead.addWidget(panel_title("项目目录"))
        wsHead.addStretch()
        ...
        origLayout.addWidget(panel_title("○ 原图"))
        ...
        infoLayout.addWidget(panel_title("○ 图像信息"))
    ...
    def _build_center(self):
        ...
        layout.addWidget(panel_title("主视图窗"))
    ...
    def _build_right_dock(self):
        ...
        host = QWidget()
        # 右键菜单：导出结果 / 导出颗粒 / 导出图表 / 智能分析（逻辑在 controllers 中连接）
        ...
        tableLayout.addWidget(panel_title("○ 统计结果"))
    ...
    def _build_bottom_dock(self):
        ...
        # ---- 处理日志（右键：清空日志 / 导出日志，逻辑在 controllers 中连接）----
