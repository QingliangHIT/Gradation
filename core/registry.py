...
约定：
    ...
    - 输出  binary: 二值图 (H, W) uint8（可为 None，无独立二值掩膜时）
...
from core.preprocess import traditional_binary
from core import watershed, unet_infer, sam_infer
...
def _run_watershed(img, p, stage):
    ...
    stage("分水岭分割中...")
    markers = watershed.segment_particles(
        binary,
...
def _run_unet(img, p, stage):
    """UNet 语义分割 + 分水岭实例分割。"""
    stage("UNet 推理中...")
    binary = unet_infer.unet_predict(img)
    stage("分水岭实例分割中...")
    markers = watershed.segment_particles_unet(
        binary,
        dist_thresh_ratio=p["dist_thresh_ratio"],
        kernel_size=p["kernel_size"],
        close_iterations=p["close_iterations"],
...
def _run_sam(img, p, stage):
    """SAM 自动实例分割。"""
    stage("SAM 模型推理中...")
    binary, markers = sam_infer.sam_segment(
        img,
...
def _run_yolo(img, p, stage):
    ...
    weights = infer_config.get_config().get("yolo_weights", "")
    ...
    stage("YOLO 推理中...")
    device = infer_config.resolve_device()
