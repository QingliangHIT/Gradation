# -*- coding: utf-8 -*-
"""UNet 预测结果交互查看器（matplotlib 窗口：浏览/缩放/拖拽/保存/指标对比）。

用法:
    python -m models.unet.viewer --model-path resunet_best.pth --img-dir particle/val/images
"""
import argparse
import os
...
from models.unet.predict import load_model, make_overlay, predict, preprocess_image
...
class PredictorApp:
    ...
    def _update_info_text(self):
        ...
        if self._mask is not None:
            iou, dice, acc = compute_metrics_numpy(self._pred, self._mask)
            text += f"\n--- Metrics ---\nIoU:  {iou:.4f}\nDice: {dice:.4f}\nAcc:  {acc:.4f}"
        else:
            text += "\n(No GT mask)"
        text += "\n\n--- Keys ---\nD   : Open directory\nF   : Open image\n+/- : Zoom\n← → : Navigate\nS   : Save"
